...
constraints were load-bearing.


% ============================================================================
\section{From Specification to Code}
\label{sec:synthesis}
% ============================================================================

\subsection{The Specification as a Blueprint}
\label{sec:synthesis-blueprint}

The fixed specification---prose amended by the two bug fixes---served as
the primary input for C and C++ implementations. The implementations
follow the phase structure of the specification one for one, apply both
bug fixes explicitly (stop-trigger timestamps are refreshed before
re-entry; the STP DECREMENT path reloads iceberg visible quantity when
it would otherwise reach zero), and include an
\texttt{engine\_check\_invariants} routine that translates the
TLC-checked invariants into runtime assertions.

The runtime assertions are not a proof. An invariant assertion fires
after state has already diverged. But they narrow the gap between
specification and implementation and have caught regressions during
development.

\subsection{What This Does and Does Not Mean}
\label{sec:synthesis-gap}

This is not a proof that the C++ code is correct. The Lean proof
establishes \texttt{AllInv} preservation for the Lean reference
implementation. It does not transfer automatically to C or C++. Those
implementations are validated by shared test suites derived from the
specification, not by a formal correspondence argument.

For the Lean reference implementation, the gap is closed: the code that
tests run against is the same code the theorems reference. There is no
extraction layer, no refinement obligation. For the systems-language
implementations, a gap remains.

A genuine end-to-end guarantee would require a refinement argument
connecting the Lean reference to the C/C++ code. That is beyond the
scope of this paper and is a natural next step.
...
The TLA+ model, the Lean~4 proofs, and the C/C++ implementations were
developed interactively with a large language model acting as a coding
assistant. Strategic decisions---the progress-lemma decomposition, the
...
The specification is 900 lines of structured pseudocode covering
...
The TLA+ specification (\texttt{MatchingEngine.tla}, approximately 850
...
of a feature-rich matching engine and rely on a separate
testing-and-assertion path to connect that reference to systems-language
code. The investment is correspondingly smaller (weeks rather than
...
programs. The approach taken in \S\ref{sec:synthesis} differs in that
the specification is both the logical source of truth and the primary
input, with architectural hints provided as orthogonal performance
directives. A formal refinement argument between the Lean reference
and the synthesized C/C++ remains future work.
...
For the C and C++ synthesized implementations, a gap remains. The Lean
proof does not transfer automatically to C or C++. Those implementations
are validated by shared test suites derived from the specification, not
by a formal correspondence argument. A refinement proof connecting the
Lean reference to the systems-language implementations is a natural
next step.
...
\item \texttt{spec/REPORT.md}---model checking output and bug
      descriptions.
\item \texttt{MatchingEngine/}---the Lean~4 project (11 source files).
      ...
      6 theorems), both under the \texttt{MatchingEngine/} directory.
      Both are built and type-checked by \texttt{lake build} and are
      imported from the library root \texttt{MatchingEngine.lean}.
...
as the primary input. The constant-fuel soundness issue described in
\S\ref{sec:lean} was encountered during this process.
